...
usage () {
    echo "Usage:" 
	echo "  df_ops [service operation] [service name] [service option]"
	echo "  df_ops [admin operation] [admin option]"
	echo ""	
	echo "CLI Para.:"
	echo "[service operation]"
	echo "  start|stop|restart: start|stop|restart df environment and service"
    echo ""	
	echo "[service]"
    ...
    echo ""		
	echo "[service option]"
	echo "  d: running df service in debug mode"
    echo ""
    echo "[admin operation]"
	echo "  status: check status of data service and environment"
	echo "  format: format all data and logs"
	echo "  help: show this help"
	echo "  admin: perform data service admin operations and must use with [admin option]"
	echo "  install: reinstall df packages and can use with [admin option]"
    echo ""
	echo "[admin option]"
	echo "  idi: reset df_installed collection. Only applicable to admin"
	echo "  branch_name: install df from specific branch name. Only applicable to install"
    echo ""
	echo "Examples:"
